...
class Reklam extends Command {
    constructor(client) {
        super(client, {
            ...
            usage: ".Reklam @Approval/ID <süre> <sebep>",
            category: "Moderation",
            aliases: ["Reklam","reklam"],
            enabled: true,
        });
    }
...
if( [PermissionsBitField.Flags.Administrator,PermissionsBitField.Flags.ManageRoles,PermissionsBitField.Flags.BanMembers,PermissionsBitField.Flags.KickMembers,].some(x=> message.member.permissions.has(x))
    ...
    let cezakontrol = await data.filter(x=> x.penaltys.some(x=> x.type == "Reklam" && x.Punished == member.id && x.Finished == false)).length > 0
