...
class Git extends Command {
  ...
  async onRequest(client, message, args, embed) {
    ...
    message.channel.send({
      content: `**${member}**`,
      embeds: [new GenerateEmbed().setDescription(`**${message.author} \`${member.voice.channel.name}\` Adlı Kanala Gelmek İstiyor, Kabul Ediyor Musun?**`)],
      components: [row]
    }).then(async msg => {
      ...
      collector.on('collect', async (button, user) => {
        if (button.customId == "evet") {
          ...
          await button.reply({ embeds: [new GenerateEmbed().setDescription(`${message.member} bulunduğunuz ses kanalına (${message.member.voice.channel}) çekildi!`)], ephemeral: true })
          if(message) await message.replyReaction(message.guild.findReaction(Tik,"ID"))
          if (msg && msg.deletable) await msg.delete();
        } else if (button.customId == "hayir") {
          await message.channel.send({ content: `**[${message.member}]**`, embeds: [new GenerateEmbed().setDescription(`${member}, bulunduğu ses kanalına (${message.member.voice.channel}) gitme isteğinizi reddetti!`)] })
          if (msg && msg.deletable) await msg.delete();
