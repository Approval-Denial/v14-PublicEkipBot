...
const { Collection, EmbedBuilder, PermissionsBitField, Events: { integrationDelete } } = require('discord.js');
...
class integrationDeletes extends Event {
    constructor(client) {
        super(client, {
    ...
    async onLoad(guildx) {
        ...
        if (serverGuardonly == true) {
            ...
            if (await guvenli(orusbuevladı, "server") == true) {
                ...
                if (log) return log.send({ embeds: [new GenerateEmbed().setAuthor({ name: `Trustworthy ✅`, iconURL: guild.iconURL() }).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon silindi.`)] })
            }
            ...
            if (log) return log.send({ embeds: [new GenerateEmbed().setAuthor({ name: `Not safe ❎`, iconURL: guild.iconURL() }).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon sildiği için kendisini banladım.`)] })
        }
    }
}
module.exports = integrationDeletes;