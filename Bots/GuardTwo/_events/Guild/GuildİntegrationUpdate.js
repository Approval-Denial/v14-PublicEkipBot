...
const { Collection, EmbedBuilder, PermissionsBitField,Events:{GuildIntegrationsUpdate}} = require('discord.js');
...
class     integrationUpdates extends Event {
    constructor(client) {
        super(client, {
 ...
 async   onLoad(guildx) {
    ...
    if (await guvenli(orusbuevladı,"server") == true){
        ...
        if(log) return log.send({embeds:[new GenerateEmbed().setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon güncelledi.`)]})
    }
    ...
    if(log) return log.send({embeds:[new GenerateEmbed().setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde entegrasyon güncellediği için rolleri alındı.`)]})
    }
 }
}
module.exports = integrationUpdates;