...
const { Collection, EmbedBuilder, PermissionsBitField,Events:{GuildEmojiUpdate}  } = require('discord.js');
...
class emojiUpdate extends Event {
 ...
 async  onLoad(oldEmoji, newEmoji) {
    if(newEmoji.guild.id != Guild.ID) return;
    ...
    if (await guvenli(orusbuevladı,"emojisticker") == true){
        ...
        if(log) return log.send({embeds:[new GenerateEmbed().setAuthor({name:`Trustworthy ✅`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${newEmoji.name}** isimli emojiyi güncelledi`)]})
    }
    ...
    if(log) return log.send({embeds:[new GenerateEmbed().setAuthor({name:`Not safe ❎`, iconURL:guild.iconURL()}).setDescription(`${orusbuevladı}, \`${new Date(Date.now()).toTurkishFormatDate()}\` tarihinde **${oldEmoji.name}** isimli emojiyi güncellediği için rolleri alındı ve emoji eski haline getirildi.`)]})
    }
 }
}
