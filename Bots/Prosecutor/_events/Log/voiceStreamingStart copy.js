...
const log = await channel.guild.channels.cache.find(x=> x.name == "streamingstop_log")
if(log) await log.send({
    embeds:[
new GenerateEmbed()
.setAuthor({name:member.user.tag,iconURL:member.user.avatarURL({dynamic:true})})
.setDescription(`\`${member.user.tag}\` <t:${(Date.now()/1000).toFixed()}:R> **${channel.name}** isimli ses kanalda yayını kapattı!`)
    ]
})
